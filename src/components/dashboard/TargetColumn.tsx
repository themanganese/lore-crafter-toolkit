...
function StatLine({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div className="flex items-baseline justify-between py-2 border-b border-gold/15 last:border-0">
      <span className="text-sm uppercase tracking-[0.2em] text-foreground/75">
        {label}
      </span>
      <span
        className={cn(
  ...
  return (
    <div className="group flex items-center gap-2.5 py-1.5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-sm text-muted-foreground italic leading-snug max-h-0 overflow-hidden group-hover:max-h-12 transition-[max-height] duration-200">
          {hook.description}
        </div>
      </div>
    </div>
  ...
  return (
    <section className="panel-grim p-6 flex flex-col gap-5 min-h-0 overflow-y-auto">
      ...
      {breakdown ? (
        ...
        <div className="h-36 flex items-center justify-center text-sm text-muted-foreground italic">
          Awaiting score…
        </div>
      )}
      ...
      <div>
        ...
        {topHooks.length > 0 ? (
          ...
          <p className="text-sm text-muted-foreground italic">None equipped.</p>
        )}
      </div>
    </section>
  );
