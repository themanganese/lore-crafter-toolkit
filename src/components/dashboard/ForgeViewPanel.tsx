      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <div className="flex items-center gap-1 cursor-default w-fit">
            <div className="font-mono text-[9px] uppercase tracking-[0.25em] text-gold-dim">
              {label}
            </div>
        ...
        {def && (
          <TooltipContent
            side="top"
            className="max-w-64 text-xs font-body leading-snug bg-card border border-gold/50 text-foreground shadow-lg"
          >
            {def}
          </TooltipContent>
        )}
    ...
    <TooltipProvider delayDuration={300}>
      <section className="panel-grim p-6 grid grid-cols-1 lg:grid-cols-[3fr_2fr] gap-6">
        ...
        <div className="flex flex-col gap-4 min-w-0">
          ...
          {forecast ? (
            <div>
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="font-mono text-[10px] uppercase tracking-[0.25em] text-gold-dim mb-1 flex items-center gap-1 cursor-default w-fit">
        ...
        <div className="flex flex-col gap-3 min-w-0">
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="font-display text-[11px] uppercase tracking-[0.4em] text-gold-dim flex items-center gap-1 cursor-default w-fit">
                Improvement Levers
                <HelpCircle className="h-2.5 w-2.5 text-gold-dim/60" />
              </div>
            </TooltipTrigger>
            ...
            <div className="flex flex-col gap-2">
              {topLevers.map((l) => (
                <Tooltip key={l.id}>
                  ...
                  <TooltipContent
                    side="left"
                    className="max-w-72 text-xs font-body leading-snug bg-card border border-gold/50 text-foreground shadow-lg"
                  >
                    <p className="font-display text-sm font-bold mb-1">{l.title}</p>
                    <p>{l.description}</p>
                    {l.patternAffected && (
                      <p className="mt-1 font-mono text-[10px] text-gold-dim uppercase tracking-widest">
