  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <button
          onClick={() => setOpen((o) => !o)}
          className="gold-frame px-3 py-1.5 font-mono text-[10px] uppercase tracking-widest text-gold-bright hover:bg-gold/10 flex items-center gap-1.5"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {character.briefs.slice(0, 6).map((b) => (
            <div key={b.id} className="border border-border rounded-sm p-3 bg-muted/15">
              ...
              <p className="font-mono text-[11px] text-foreground/70 mt-2 line-clamp-2">
                {b.prompt}
              </p>
            </div>
          ))}
      ...
      {open && (
        <div className="border border-border rounded-sm p-4 space-y-4 bg-muted/10">
          ...
          <button
            ...
            className="w-full btn-copper px-4 py-2.5 font-display tracking-wider text-sm rounded-sm disabled:opacity-50"
          >
            Save brief
          </button>
        </div>
